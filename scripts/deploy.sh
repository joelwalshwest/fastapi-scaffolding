...
if [ "$ENV" == "prod" ]; then
    ...
    docker build -t joelwalshwest/fastapi-scaffolding:$HASH . --target slim --platform=linux/amd64 --no-cache
    docker push joelwalshwest/fastapi-scaffolding:$HASH
    gcloud run deploy qa-fastapi-scaffolding --image joelwalshwest/fastapi-scaffolding:$HASH --platform managed  --allow-unauthenticated --project fastapi-scaffolding --set-env-vars="ENVIRONMENT=qa"
fi